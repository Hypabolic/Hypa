...
public sealed class SqliteArtifactRepository(HypaDataOptions options, SqliteSchemaInitializer schema) : IArtifactRepository
{
    public async Task<Result<ArtifactRef, Error>> StoreAsync(string content, string mimeType, Guid sessionId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (!init.IsOk) return Result<ArtifactRef, Error>.Fail(StorageFailure.ToError(init.Error));

            Directory.CreateDirectory(options.ArtifactsDirectory);

            var id = Guid.NewGuid();
            var bytes = Encoding.UTF8.GetBytes(content);
            storagePath = Path.Combine(options.ArtifactsDirectory, id.ToString());
            ...
            catch (Exception ex) when (StorageFailure.IsExpected(ex))
            {
                ...
                return Result<ArtifactRef, Error>.Fail(StorageFailure.ToError(ex));
            }
    ...
    public async Task<Result<string, Error>> LoadAsync(Guid artifactId, CancellationToken ct)
    {
        ...
        {
            var init = await schema.InitAsync(ct);
            if (!init.IsOk) return Result<string, Error>.Fail(StorageFailure.ToError(init.Error));
            await using var conn = new SqliteConnection($"Data Source={options.DatabasePath}");
            await conn.OpenAsync(ct);
            await using var cmd = conn.CreateCommand();
        ...
        {
            return Result<string, Error>.Fail(StorageFailure.ToError(ex));
        }
    }
    ...
    public async Task<Result<IReadOnlyList<ArtifactRef>, Error>> ListForSessionAsync(Guid sessionId, CancellationToken ct)
    {
        ...
        {
            var init = await schema.InitAsync(ct);
            if (!init.IsOk) return Result<IReadOnlyList<ArtifactRef>, Error>.Fail(StorageFailure.ToError(init.Error));
            await using var conn = new SqliteConnection($"Data Source={options.DatabasePath}");
            await conn.OpenAsync(ct);
            await using var cmd = conn.CreateCommand();
        ...
        {
            return Result<IReadOnlyList<ArtifactRef>, Error>.Fail(StorageFailure.ToError(ex));
        }
    }
