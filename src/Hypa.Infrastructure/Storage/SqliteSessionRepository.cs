...
public sealed class SqliteSessionRepository(HypaDataOptions options, SqliteSchemaInitializer schema) : ISessionRepository
{
    public async Task<Result<ContextSession, Error>> LoadAsync(Guid id, CancellationToken ct)
    {
        try
        {
            ...
            if (!init.IsOk) return Result<ContextSession, Error>.Fail(StorageFailure.ToError(init.Error));
        ...
        {
            return Result<ContextSession, Error>.Fail(StorageFailure.ToError(ex));
        }
    }
    ...
    public async Task<Result<IReadOnlyList<ContextSession>, Error>> ListForProjectAsync(string projectRoot, CancellationToken ct)
    {
        ...
        {
            var init = await schema.InitAsync(ct);
            if (!init.IsOk) return Result<IReadOnlyList<ContextSession>, Error>.Fail(StorageFailure.ToError(init.Error));
            await using var conn = OpenConnection();
            await conn.OpenAsync(ct);
            await using var cmd = conn.CreateCommand();
        ...
        {
            return Result<IReadOnlyList<ContextSession>, Error>.Fail(StorageFailure.ToError(ex));
        }
    }
